...
fn example() {
    ...
    f(); // will print 🍓, f now owns the reference to s

    let v = vec![2, 4, 6];
    v.iter()
        .map(|x| x * 3) // map((x) => x * 3)
        .filter(|x| *x > 10) // filter((x) => x > 10) the *x dereferences the x variable
        .fold(0, |acc, x| acc + x); // reduce(((acc, x) => acc + x), 0)
}

fn expensive_sum(v: Vec<i32>) -> i32 {
    pause_ms(500);
    println!("Child thread: just about finished");
    // In the closures for both the .filter() and .map() the argument will be a reference, so you'll either need to dereference the argument once in the parameter list like this: `|&x|` or you will need to dereference it each time you use it in the expression like this: `*x`
...
fn main() {
    ...
    // Time for some fun with threads and channels!  Though there is a primitive type of channel in the std::sync::mpsc module, I recommend always using channels from the crossbeam crate, which is what we will use here.
    //
    // Examine how the flow of execution works.  Once you understand it, alter the values passed to the `pause_ms()` calls so that both the "Thread B" outputs occur before the "Thread A" outputs.

    let (tx, rx) = channel::unbounded();
    // Cloning a channel makes another variable connected to that end of the channel so that you can send it to another thread.
    let tx2 = tx.clone();
